...
// src/vitest-plugin/compose-initial-globals.ts
var composeInitialGlobals = ({
  userInitialGlobals,
  runConfig,
  ghostStoriesEnabled,
  ...
  let globals = { ...userInitialGlobals };
  return globals[STORYBOOK_TEST_PROVIDE_KEY] = runConfig, ghostStoriesEnabled ? globals.ghostStories = { enabled: !0 } : delete globals.ghostStories, renderAnalysisEnabled ? globals.renderAnalysis = { enabled: !0 } : delete globals.renderAnalysis, globals.a11y = {
    ...globals.a11y,
...
var convertToFilePath = (url) => url.replace(/^file:\/\//, "").replace(/^\/+([a-zA-Z]:)/, "$1").replace(/%20/g, " "), testStory = ({
  ...
  let userInitialGlobals = {};
  try {
    userInitialGlobals = inject(STORYBOOK_TEST_INITIAL_GLOBALS_PROVIDE_KEY) ?? {};
  } catch {
  }
  let shouldRunA11yTests = !!runConfig.a11y, initialGlobals = composeInitialGlobals({
    userInitialGlobals,
    runConfig,
    ghostStoriesEnabled,
    renderAnalysisEnabled,
